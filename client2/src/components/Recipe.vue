<template>
    <div class="recipe" v-bind:class="{'no-matches': showMatchCount && matchCount === 0}">
        ...
        <router-link 
            class="clickable" 
            v-bind:class="{'no-hover': dontLink}"
            v-bind:to="dontLink ? '#' : '/recipes/recipe/' + id" 
            @mouseenter="hover = true" 
            @mouseleave="hover = false">

            <img v-bind:src="imgURL" v-bind:class="{'small-img': small}" width="100%">

            <div class="recipe-details" v-bind:class="{'recipe-details-small': small}">
                <div v-if="!small" class="d-flex justify-content-between align-items-start">
                ...
                <div v-else>
                    ...
                    <span class="d-flex justify-content-start align-items-center">
                        <Stars v-bind:stars="rating" max="5" starSize="9pt"></Stars>
                        <span class="number-of-ratings muted ml-2">({{numOfRatings}})</span>
                    </span>
                </div>

                <div class="description normalFont" v-if="!small">
                    {{description}}
                </div>

                <div class="time normalFont d-flex justify-content-between align-items-center" v-if="!small">
